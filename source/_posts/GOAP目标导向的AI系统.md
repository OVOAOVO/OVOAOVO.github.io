...
# （重要）补充说明
后面复习这部分的时候重新拾起来总算把之前的问题都解决掉了，卡掉的部分在Game104详细说明了，附跳转链接
>https://www.bilibili.com/video/BV1iG4y1i78Q/

## 建图部分Games104详细说了，以goal/state为节点，action作为边，distance作为cost

## 如何去处理action的编排以及查找方式也说了，使用一个栈存下所有当前要达成goal的未满足state，再去查找未满足state对应的action的Effect state，直到这个栈能清空为止，即为完成
